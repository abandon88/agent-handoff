...
每次收尾后，除了用于 AI 恢复上下文的 `SESSION-LOG.md`，还会自动生成一份更适合人看的 `DAILY-LOG.md`。它会按日期把当天多次会话合并到同一段里，只保留本次主题、完成事项和风险备注，方便你回头统计工作量、写周报或者复盘当天到底推进了什么。
...
* 新增 `DAILY-LOG.md` 每日工作日志：每次收尾后自动按日期汇总当天做了什么。

* 同一天多次会话会合并到同一个日期段，方便回看一天内连续推进的工作。

* 每日工作日志只保留主题、完成事项和风险备注，不展示下次恢复点和涉及文件，更适合统计工作量、写周报和复盘。

* `DAILY-LOG.md` 已接入初始化、收尾、重建和校验流程，展示文件坏了也能从 `_state/` 重新生成。
